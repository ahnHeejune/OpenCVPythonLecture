...
# TF 설치시 빠진 경우가 있으니. 그 경우는 github에서 다운 받아서 본인은 환경에 설치 해야함.
...
#    Conv     -> (?, 28, 28, 32) # spatial 28x28, out channel pattern = 32 
#    Pool     -> (?, 14, 14, 32)
...
# 2-D filter of 3x3, with 32 inputs, and 64 outputs 
...
for epoch in range(training_epochs):
    ...
    '''Weight = W1.eval(session=sess)
    for wi in range(10):   
       image = Weight[:, wi].reshape([7*8,7*8])
       plt.title('W1 for %d' % (wi+1))
